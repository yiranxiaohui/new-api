...
// PinHostOwnedPluginRoute binds a host-registered path to a factory plugin's
// declared native route by (method, path). It exists for vendor entry points
// whose path shape collides with a host protocol operation, so the plugin
// router cannot register them itself. The plugin is resolved from the live
// generation on every request, so upgrades and disables take effect without
// a restart; a missing or disabled plugin yields the standard task error.
func PinHostOwnedPluginRoute(pluginKey string, route pluginruntime.Route) gin.HandlerFunc {
	return func(c *gin.Context) {
		generation := pluginruntime.DefaultRegistry.Generation()
		plugin, ok := generation.Get(pluginKey)
		if !ok {
			code, message := hostOwnedPluginRouteUnavailable(pluginKey)
			c.AbortWithStatusJSON(http.StatusBadRequest, &dto.TaskError{Code: code, Message: message, StatusCode: http.StatusBadRequest})
			return
		}
		...
		logger.LogDebug(
			c,
			"task_plugin subsystem=router event=host_route_matched generation=%d plugin=%q version=%q method=%q route_type=%q",
			generation.Number,
			plugin.Meta.Key,
			plugin.Meta.Version,
			route.Method,
...
func hostOwnedPluginRouteUnavailable(pluginKey string) (string, string) {
	...
	return "task_plugin_disabled", fmt.Sprintf("task plugin %q is disabled on this gateway", pluginKey)
}
