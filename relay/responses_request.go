...
func removeUnsupportedPreviousResponseID(jsonData []byte, info *relaycommon.RelayInfo) ([]byte, error) {
	if !shouldRemoveUnsupportedPreviousResponseID(info) || !gjson.GetBytes(jsonData, "previous_response_id").Exists() {
		return jsonData, nil
	}
...
func PrepareResponsesRequest(c *gin.Context, info *relaycommon.RelayInfo, req *dto.OpenAIResponsesRequest) (relaychannel.Adaptor, common.ReplayableBody, io.Closer, *types.NewAPIError) {
	...
	if model_setting.GetGlobalSettings().PassThroughRequestEnabled || info.ChannelSetting.PassThroughBodyEnabled {
		...
		if shouldRemoveUnsupportedPreviousResponseID(info) && strings.TrimSpace(request.PreviousResponseID) != "" {
			...
			payload, err = removeUnsupportedPreviousResponseID(payload, info)
			if err != nil {
	...
	// Persistent WebSocket connections support continuation; stateless HTTP
	// subscription channels retain the fork's compatibility rewrite.
