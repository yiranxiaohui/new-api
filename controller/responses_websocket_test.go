...
func TestResponsesWSRequestRunnerHonorsUserConcurrencyAndReleasesSlot(t *testing.T) {
	require.NoError(t, i18n.Init())
	user, token := setupResponsesWSRequestTest(t)
	previous := setting.UserMaxConcurrency
	setting.UserMaxConcurrency = 1
	t.Cleanup(func() { setting.UserMaxConcurrency = previous })
	runner, _ := newResponsesWSTestRunner(t, token)
	allowed, release := service.TryAcquireUserConcurrency(user.Id, 1)
	require.True(t, allowed)
	t.Cleanup(release)
	called := false
	handle := func(*gin.Context) *types.NewAPIError { called = true; return nil }
	apiErr := runner(httptest.NewRequest(http.MethodPost, "/v1/responses", nil), "blocked", handle)
	require.NotNil(t, apiErr)
	assert.Equal(t, http.StatusTooManyRequests, apiErr.StatusCode)
	assert.False(t, called)
	release()
	require.Nil(t, runner(httptest.NewRequest(http.MethodPost, "/v1/responses", nil), "released", handle))
	assert.True(t, called)
	allowed, releaseAfter := service.TryAcquireUserConcurrency(user.Id, 1)
	require.True(t, allowed, "completed WebSocket requests release the shared user slot")
	releaseAfter()
}

func TestResponsesWSRequestRunnerPreservesChannelErrorMasking(t *testing.T) {
	for _, hide := range []bool{false, true} {
		t.Run(fmt.Sprintf("hide=%t", hide), func(t *testing.T) {
			_, token := setupResponsesWSRequestTest(t)
			runner, _ := newResponsesWSTestRunner(t, token)
			apiErr := runner(httptest.NewRequest(http.MethodPost, "/v1/responses", nil), "masked", func(c *gin.Context) *types.NewAPIError {
				common.SetContextKey(c, constant.ContextKeyChannelSetting, dto.ChannelSettings{HideUpstreamErrors: hide, HideUpstreamErrorMessage: "Provider unavailable"})
				return types.WithOpenAIError(types.OpenAIError{Message: "private upstream detail", Type: "upstream_error"}, http.StatusBadGateway)
			})
			require.NotNil(t, apiErr)
			assert.Equal(t, http.StatusBadGateway, apiErr.StatusCode)
			message := apiErr.ToOpenAIError().Message
			if hide {
				assert.Equal(t, "Provider unavailable", message)
			} else {
				assert.Contains(t, message, "private upstream detail")
			}
		})
	}
}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			fixture := newResponsesWSBillingTest(t, `tier("request", fixed(0.002))`, func(ws *websocket.Conn, r *http.Request) {
				...
				var create dto.OpenAIResponsesRequest
				if !assert.NoError(t, common.Unmarshal(payload, &create)) {
					return
				}
				assert.Equal(t, "resp_previous", create.PreviousResponseID)
				if !assert.NoError(t, ws.WriteMessage(websocket.TextMessage, []byte(terminal))) {
					return
				}
				_, _, _ = ws.ReadMessage()
			...
			fixture.httpUpstream = func(w http.ResponseWriter, r *http.Request) {
				observe(r)
				var create dto.OpenAIResponsesRequest
				if !assert.NoError(t, common.DecodeJson(r.Body, &create)) {
					return
				}
				if tc.channelType == constant.ChannelTypeSub2API {
					assert.Empty(t, create.PreviousResponseID)
				} else {
					assert.Equal(t, "resp_previous", create.PreviousResponseID)
				}
				w.Header().Set("Content-Type", "text/event-stream")
			...
			require.NoError(t, fixture.client.WriteMessage(websocket.TextMessage, []byte(`{"type":"response.create","model":"ws-billing","input":"hi","previous_response_id":"resp_previous"}`)))
			...
			request, err := http.NewRequest(http.MethodPost, fixture.gatewayURL+"/v1/responses", strings.NewReader(`{"model":"ws-billing","input":"hi","stream":true,"previous_response_id":"resp_previous"}`))
			require.NoError(t, err)
			request.Header.Set("Authorization", "Bearer sk-"+fixture.token.Key)
			request.Header.Set("Content-Type", "application/json")
			response, err := (&http.Client{Timeout: 3 * time.Second}).Do(request)
