...
test('preserves fork concurrency and error masking settings in the reorganized editor', async () => {
  editingChannel = {
    ...editingChannel,
    max_concurrency: 3,
    setting:
      '{"hide_upstream_errors":true,"hide_upstream_error_message":"Provider unavailable"}',
  }
  const put = vi
    .spyOn(api, 'put')
    .mockResolvedValue({ data: { success: true } })
  ...
  expect(routing).toHaveAccessibleName(/Configured/)
  ...
  const request = screen.getByRole('tab', { name: /Request & Response/ })
  expect(request).toHaveAccessibleName(/Configured/)
  await user.click(request)
  expect(
    screen.getByRole('switch', { name: 'Hide Upstream Errors' })
  ).toBeChecked()
  expect(screen.getByLabelText('Custom Error Message')).toHaveValue(
  ...
  expect(JSON.parse(payload.setting)).toMatchObject({
    ...
    hide_upstream_error_message: 'Try later',
  })
})
